...
import io.iohk.ethereum.domain.{Block, BlockHeader, BlockchainImpl, SignedBlockHeader}
...
class BlockQueueSpec extends FlatSpec with Matchers with MockFactory {
  ...
  "BlockQueue" should "ignore block if it's already in the queue" in new TestConfig {
    ...
    blockQueue.enqueueBlock(block) shouldEqual Some(Leaf(block.signedHeader.hash, block.signedHeader.header.difficulty))
    blockQueue.enqueueBlock(block) shouldEqual None
    blockQueue.isQueued(block.signedHeader.hash) shouldBe true
  }
  ...
  it should "ignore blocks outside of range" in new TestConfig {
    ...
    blockQueue.isQueued(block1.signedHeader.hash) shouldBe false

    blockQueue.enqueueBlock(block30)
    blockQueue.isQueued(block30.signedHeader.hash) shouldBe false
  }
  ...
  it should "remove the blocks that fall out of range" in new TestConfig {
    ...
    blockQueue.enqueueBlock(block1)
    blockQueue.isQueued(block1.signedHeader.hash) shouldBe true
    ...
    blockQueue.isQueued(block20.signedHeader.hash) shouldBe true
    blockQueue.isQueued(block1.signedHeader.hash) shouldBe false
  }
  ...
  it should "enqueue a block with parent on the main chain updating its total difficulty" in new TestConfig {
    ...
    blockQueue.enqueueBlock(block1) shouldEqual Some(Leaf(block1.signedHeader.hash, block1.signedHeader.header.difficulty + 42))
  }
  ...
  it should "enqueue a block with queued ancestors rooted to the main chain updating its total difficulty" in new TestConfig {
    ...
    val block2a = getBlock(2, 102, block1.signedHeader.hash)
    val block2b = getBlock(2, 99, block1.signedHeader.hash)
    val block3 = getBlock(3, 103, block2a.signedHeader.hash)
    ...
    val expectedTd = 42 + List(block1, block2a, block3).map(_.signedHeader.header.difficulty).sum
    blockQueue.enqueueBlock(block3) shouldEqual Some(Leaf(block3.signedHeader.hash, expectedTd))
  }
  ...
  it should "remove a branch from a leaf up to the first shared ancestor" in new TestConfig {
    val block1 = getBlock(1)
    val block2a = getBlock(2, parent = block1.signedHeader.hash)
    val block2b = getBlock(2, parent = block1.signedHeader.hash)
    val block3 = getBlock(3, parent = block2a.signedHeader.hash)
    ...
    blockQueue.getBranch(block3.signedHeader.hash, dequeue = true) shouldEqual List(block1, block2a, block3)

    blockQueue.isQueued(block3.signedHeader.hash) shouldBe false
    blockQueue.isQueued(block2a.signedHeader.hash) shouldBe false
    blockQueue.isQueued(block2b.signedHeader.hash) shouldBe true
    blockQueue.isQueued(block1.signedHeader.hash) shouldBe true
  }
  ...
  it should "remove a whole subtree down from an ancestor to all its leaves" in new TestConfig {
    ...
    val block2a = getBlock(2, parent = block1a.signedHeader.hash)
    val block2b = getBlock(2, parent = block1a.signedHeader.hash)
    val block3 = getBlock(3, parent = block2a.signedHeader.hash)
    ...
    blockQueue.isQueued(block3.signedHeader.hash) shouldBe true
    blockQueue.isQueued(block2a.signedHeader.hash) shouldBe true
    blockQueue.isQueued(block2b.signedHeader.hash) shouldBe true
    blockQueue.isQueued(block1a.signedHeader.hash) shouldBe true
    blockQueue.isQueued(block1b.signedHeader.hash) shouldBe true

    blockQueue.removeSubtree(block1a.signedHeader.hash)

    blockQueue.isQueued(block3.signedHeader.hash) shouldBe false
    blockQueue.isQueued(block2a.signedHeader.hash) shouldBe false
    blockQueue.isQueued(block2b.signedHeader.hash) shouldBe false
    blockQueue.isQueued(block1a.signedHeader.hash) shouldBe false
    blockQueue.isQueued(block1b.signedHeader.hash) shouldBe true
  }
  ...
  trait TestConfig {
    ...
    def setTotalDifficultyForParent(block: Block, td: Option[BigInt] = None) =
      (blockchain.getTotalDifficultyByHash _).expects(block.signedHeader.header.parentHash).returning(td)
