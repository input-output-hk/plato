...
import io.iohk.ethereum.domain.{Block, BlockHeader, SlotTimestampConverter}
...
class OmmersValidatorSpec extends FlatSpec with Matchers with PropertyChecks with ObjectGenerators with MockFactory{
  ...
  val slotCalculatorMock = mock[SlotTimestampConverter]
  ...
  it should "validate correctly a valid list of ommers" in new BlockUtils {
    ...
    (slotCalculatorMock.getSlotStartingTime _).expects(*).returning(System.currentTimeMillis() - 1000).anyNumberOfTimes()
